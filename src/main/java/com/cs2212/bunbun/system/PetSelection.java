...
public class PetSelection extends JPanel {
    ...
    private void showLoadingScreenAndSwitchPanel(CardLayout cardLayout, JPanel mainPanel, String targetPanel, String petName, String selectedPet) {
        ...
        Timer loadingTimer = new Timer(2000, e -> {
            // Stop any animations or timers if needed
            if ("Gameplay".equals(targetPanel)) {
                // Find the Gameplay panel and update it
                for (Component comp : mainPanel.getComponents()) {
                    if (comp instanceof Gameplay) {
                        Gameplay gameplayPanel = (Gameplay) comp;
                        gameplayPanel.updatePetInfo(petName, selectedPet); // Update Gameplay with pet info
                        break;
                    }
                }
                cardLayout.show(mainPanel, "Gameplay"); // Switch to Gameplay
            } else {
                cardLayout.show(mainPanel, targetPanel); // Switch to another panel
            }
